				       ...
				       char *name)
{
	...
	printk("########################s5k4ecgx_sensor_burst_write\n");
...
void s5k4ecgx_check_REG_TC_GP_EnablePreviewChanged(void)
{
	...
	printk("[S5K4ECGX]kavitha s5k4ecgx_check_REG_TC_GP_EnablePreviewChanged\n");
	while (cnt < 150) {
		s5k4ecgx_sensor_write(0x002C, 0x7000);
		s5k4ecgx_sensor_write(0x002E, 0x0244);
		s5k4ecgx_sensor_read(0x0F12, (unsigned short *)
				     &REG_TC_GP_EnablePreviewChanged);
		printk("#############&REG_TC_GP_EnablePreviewChanged:%d\n",REG_TC_GP_EnablePreviewChanged);
	...
	if (cnt)
		printk(" [S5K4ECGX] wait time for preview frame : %dms\n",
			cnt * 10);
	if (REG_TC_GP_EnablePreviewChanged)
		printk(" [S5K4ECGX] start preview failed.\n");
}
...
void cam_pw(int status)
{
	...
	static struct regulator *vreg_ldo15;
	...
	if (status == 1) {
		...
		vreg_ldo17 = regulator_get(NULL, "ldo17");
		if (IS_ERR(vreg_ldo17)) {
		//if (!vreg_ldo17) {
			printk("#####[s5k54ecgx]%s: VREG L17 get failed\n",
			__func__);
			}
		vreg_ldo15 = regulator_get(NULL, "ldo15");
		if (IS_ERR(vreg_ldo15)) {
		//if (!vreg_ldo15) {
		printk("############[s5k54ecgx]%s: VREG L15 get failed\n",
			__func__);
		}
		...
		//if (vreg_set_level(vreg_ldo17, 2800)) {
			printk("####[s5k54ecgx]%s: vreg_set_level failed\n",
			__func__);
			}
		ret = regulator_enable(vreg_ldo17);
		if (ret) {
			printk("######![s5k54ecgx]%s: reg_enable failed\n",
			__func__);
			}
		usleep(1000);
	...
	//vreg_ldo15 = vreg_get(NULL, "vcamio");
		...
		ret = regulator_set_voltage(vreg_ldo15,1800000,1800000);
		if (ret) {
		printk("[s5k54ecgx]%s: vreg_set_level failed\n",
			__func__);
	}
		ret = regulator_enable(vreg_ldo15);
		if (ret) {
		printk("#############![s5k54ecgx]%s: reg_enable failed\n",
			__func__);
	}
...
{
	printk("s5k4ecgx_set_sensor_mode : %d\n", mode);
	switch (mode) {
	case SENSOR_PREVIEW_MODE:
...
{
	int rc = 0;
	printk("###############kkkkkkkkkkk s5k4ecgx_i2c_probe\n");
				 ...
				 struct msm_sensor_ctrl *s)
{
	...
	printk("##############kkkkkkkkkkkkk s5k4ecgx_sensor_probe\n");
	...
	printk("#########lemme power on sensor n see\n");
	cam_pw(1);
	usleep(10*1000);
/*while(i<1000)
{
