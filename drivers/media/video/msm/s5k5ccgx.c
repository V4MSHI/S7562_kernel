...
#include "s5k5ccgx_torino.h"/*Torino*/
#else
...
void s5k5ccgx_set_power(int status)
{
	...
	if (status == CAM_POWER_ON) {
		...
		cam_ldo_power_on();
		/*
		msm_camio_clk_enable(CAMIO_CAM_MCLK_CLK); ::eunice09.kim : Change Power sequence
		msm_camio_clk_rate_set(24000000);
		udelay(20); // >10us (temp)
...
{
	int rc = 0;
	//eunice09 : change power sequence for Kyle i
	...
	//s5k5ccgx_set_power(CAM_POWER_ON); eunice09: change power sequence
	/*msleep(10);*/
	usleep(10*1000);
	/*
...
init_fail:
	//s5k5ccgx_set_power(CAM_POWER_OFF);     //eunice09: change power sequence
	kfree(s5k5ccgx_ctrl);
	return rc;
}
...
int s5k5ccgx_sensor_release(void)
{
	...
	//s5k5ccgx_set_power(CAM_POWER_OFF); : eunice09.kim : change power sequence for Kyle i
	gpio_set_value(CAM_MEGA_RST, 0);
	udelay(70);
		...
		struct msm_sensor_ctrl *s)
{
	...
	//eunice09 : Change Power sequence
	mdelay(20);
	msm_camio_clk_enable(CAMIO_CAM_MCLK_CLK);
	msm_camio_clk_rate_set(24000000);
	mdelay(10);
	...
	s->s_power = s5k5ccgx_set_power;  //eunice09.kim for Kylei
...
probe_done:
	msm_camio_clk_disable(CAMIO_CAM_MCLK_CLK); //eunice09.kim  :Change Power sequence for Kylei
	s5k5ccgx_set_power(CAM_POWER_OFF);
