...
*/
	...
	nRet = mms100_ISC_download( (const UINT8*) MELFAS_binary, (const UINT16)MELFAS_binary_nLength , 0);
	if (nRet)
		goto fw_error;
#endif
#if MELFAS_ISC_2CHIP_DOWNLOAD_ENABLE
	nRet = mms100_ISC_download( (const UINT8*) MELFAS_binary_2, (const UINT16)MELFAS_binary_nLength_2, 1); // Slave Binary data download
	if (nRet)
		goto fw_error;
#endif
#endif
...
int mms100_ISC_download_binary_file(void)
{
	...
	UINT8  *pBinary[2] = {&MELFAS_binary,NULL};
	UINT16 nBinary_length[2] ={MELFAS_binary_nLength ,0};
...
static void mcsdl_i2c_start(void)
{
	gpio_direction_output(GPIO_TOUCH_I2C_SDA, 1);mcsdl_delay(MCSDL_DELAY_1US);
	gpio_direction_output(GPIO_TOUCH_I2C_SCL, 1);mcsdl_delay(MCSDL_DELAY_1US);
...
static void mcsdl_i2c_stop(void)
{
	gpio_direction_output(GPIO_TOUCH_I2C_SCL, 0);mcsdl_delay(MCSDL_DELAY_1US);
	gpio_direction_output(GPIO_TOUCH_I2C_SDA, 0);mcsdl_delay(MCSDL_DELAY_1US);
...
static void mms100_ISC_set_ready(void)
{
	...
	gpio_direction_output(GPIO_TOUCH_I2C_SDA, 1);
	MCSDL_GPIO_SDA_SET_HIGH();

	gpio_direction_output(GPIO_TOUCH_I2C_SCL, 1);
