...
static int disc_count_for_force_sleep;
...
static void if_usb_disconnect(struct usb_interface *intf)
{
	...
	if (usb_ld->dev_count == 0) {
		...
		pr_info("%s : disc_count_for_force_sleep is %d\n",
			__func__, disc_count_for_force_sleep);
		if (disc_count_for_force_sleep >= 1) {
			pr_info("%s : USB3333_RESET\n", __func__);
			gpio_direction_output(GPIO_USB3333_RESETB, 0);
			gpio_set_value(GPIO_USB3333_RESETB, 0);
		}
		disc_count_for_force_sleep++;
