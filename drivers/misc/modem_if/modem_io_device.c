...
int init_io_device(struct io_device *iod)
{
	...
	case IODEV_NET:
		...
		pr_debug("[IOD] <%s> (iod:0x%p)\n", __func__, iod);
		vnet = netdev_priv(iod->ndev);
		pr_debug("[IOD] <%s> (vnet:0x%p)\n", __func__, vnet);
		vnet->iod = iod;
	...
	pr_debug("[IOD] <%s> %s(%d) : init_io_device() done : %d\n",
