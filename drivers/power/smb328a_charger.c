...
static void _smb328a_custom_charger(void)
{
	...
	/* OVP to normal volatage charge current issue */
...
static int _smb328a_set_fast_charge_current(enum smb328a_current_t curr)
{
	...
	if (curr == smb328a_pdata->chg_curr_usb)
		ac_limit = smb328a_pdata->chg_ac_limit_usb;
	else
		ac_limit = smb328a_pdata->chg_ac_limit_ta;
...
static int _smb328a_set_termination_current(void)
{
	static char *term_text[] = {
        ...
        int chg_term_curr = smb328a_pdata->chg_term_curr;
