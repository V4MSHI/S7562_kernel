...
struct pdp_info {
	...
	union {
		...
		struct {
			/* CSD, ROUTER, GPS, XGPS, CDMA, SMD, CPLOG, LOOPBACK */
			struct tty_driver tty_driver[8];
			int refcount;
...
static int multipdp_vs_read(struct pdp_info *dev, char *buf, size_t len)
{
	...
	if (len > MAX_RX_BUFF_LEN) {
		...
		if (dev->vs_dev.refcount == 0)
			pr_err("[MULTIPDP] REFCOUNT is NULL : (1B)~ !!!!\n");
		...
		if (dev->vs_dev.refcount == 0)
			pr_err("[MULTIPDP] REFCOUNT is NULL : (2B)~ !!!!\n");
