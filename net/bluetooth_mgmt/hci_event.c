...
static inline void hci_auth_complete_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	#if defined(CONFIG_BT_CSR_7820) && !defined(CONFIG_MACH_KYLE_I)
	if (ev->status == 0x06) {
		struct hci_cp_auth_requested cp;
		hci_remove_link_key(hdev, &conn->dst);
		cp.handle = cpu_to_le16(conn->handle);
	...
	if (ev->status == 0x17 && hdev->ssp_mode > 0 &&
			conn->ssp_mode > 0) {
		struct hci_cp_auth_requested cp;
		hci_remove_link_key(hdev, &conn->dst);
		cp.handle = cpu_to_le16(conn->handle);

		BT_ERR("Repeated Attempts !!!");
		setup_timer(&conn->repeatedattempts_timer,
		hci_event_auth_repeated_attempts_timeout, (unsigned long) conn);
